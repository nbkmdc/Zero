...
export const getZeroDB = async (userId: string) => {
  return new ZeroDB(userId);
};

export const getZeroDriver = async (connectionId: string) => {
  const stub = durableObjects.ZERO_DRIVER.get(connectionId);
  await stub.setMetaData(connectionId);
...
export const getZeroSocketAgent = async (connectionId: string) => {
  const stub = durableObjects.ZERO_AGENT.get(connectionId);
  return stub;
};
